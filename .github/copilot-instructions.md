...
- `UnwrapChangelogEntries(lines) []string` -- joins a bullet with its continuation lines into one physical line, over the whole document (not only `[Unreleased]`); always runs before `SortChangelogEntries`
