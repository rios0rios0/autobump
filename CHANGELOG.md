...
- declared test files as test sources for SonarCloud Automatic Analysis in `.sonarcloud.properties`, so duplicated setup in `**/*_test.go` and the support packages under `test/` no longer fails the "Duplication on New Code" quality gate
...
- layered the configuration. AutoBump now folds four sources, each overriding only the keys its own document declares: the built-in defaults it ships with (embedded in the binary, so it knows every language it supports with no configuration and no network), the same file fetched from `main` on a best effort so a language fix reaches an installed binary without a release, the operator's own file, and the `.autobump.yaml` inside the repository being released. Only the operator's file may name a credential, a `providers`/`projects` list or the bump branch prefix; the other three decode through a schema that has no field for them, so those keys have nowhere to land rather than being caught by a check that has to run at the right moment.
- validated `bump_branch_prefix` before anything uses it. The prefix decides which branches stale-branch cleanup deletes, and an operator's typo is as capable of a mass deletion as a hostile repository would be, so it must name a branch git accepts, must contain a `/`, and must not stop at one — `chore/` matches `chore/autoupdate-*`, which is how one tool ends up deleting another's branches and closing its pull requests. A bad prefix fails at startup, before a single branch has been listed. `filterStaleBumpBranches` carries the second lock: it never deletes a branch equal to the prefix itself.

### Changed

- **BREAKING CHANGE:** narrowed what a repository's own `.autobump.yaml` may set. `providers`, `projects` and every credential key now decode through a struct that has no field for them, and so does `bump_branch_prefix` — which is not a naming preference but the argument to a destructive operation, since stale-branch cleanup deletes every remote branch carrying it and closes each attached pull request. A repository that sets one is told it was ignored and why. Everything else layers freely, which is new: a project's file now wins over the `projects[]` entry an operator wrote for it, where before it could only fill in what that entry left empty.
- **BREAKING CHANGE:** replaced `refresh_commands` with a boolean `refresh`. AutoBump now owns the command: a configuration says *whether* to regenerate what the version files derive, never *what to run*, and the recipes are compile-time constants of the program. That is what made the layering above possible. An argv read from configuration is an executable run with the release credentials, so a repository's own file could never be trusted with it — and that single exception is what stopped a project's configuration from simply overriding things the way every other layer does. Set `refresh: true` at the top level or under a language; the package manager is detected from the repository (`packageManager` first, then the lockfiles) and only JavaScript workspaces have a recipe, because nothing AutoBump rewrites elsewhere invalidates a lockfile. Yarn Classic is skipped with a warning: it has no install mode that resolves without also linking and running install scripts. An operator carrying the old key is told by name what to write instead.
- **BREAKING CHANGE:** split `configs/autobump.yaml` into the defaults AutoBump ships and `configs/autobump.example.yaml`. The shipped file is now merged into every run, so it can no longer carry the placeholder credentials and the example `projects` list it used to: `gitlab_access_token: 'glpat-TOKEN'` would have become the *effective* token for every operator who keeps none of their own, shadowing `CI_JOB_TOKEN` in GitLab CI, and the three example paths would have made `autobump run` walk directories that belong to the documentation. Copy the example file instead.
- changed the Go module dependencies to their latest versions

### Fixed

- honoured the global `changelog_path`. It was decoded and never read, so the setting the README documented as applying to every project did nothing; `resolveChangelogPath` now falls back to it when the project entry does not name one.
- stopped refreshing a Yarn Classic project with npm. Detecting Classic now ends the search rather than falling through to the next lockfile marker: a repository carrying a v1 `yarn.lock` beside a stale `package-lock.json` is still a Yarn project, and the refresh would have written it a lockfile it has no business carrying.
- stopped resolving the global configuration from the working directory. AutoBump normally runs with the repository it is releasing as the working directory, and that repository may carry its own `.autobump.yaml` for per-project overrides — so when the operator kept no configuration in their home directory, the project's file was adopted as the *global* one. That is not a reordered preference, it is a substitution, and three things followed from it. The project's overrides stopped being overrides: they are meant to be layered onto the global configuration, and as the global configuration they replaced AutoBump's published defaults for every other language along with them. `SanitizeUntrustedLanguages` never ran, so refresh commands declared by the project were honoured — they are executables run with the release credentials, and the project path strips them for exactly that reason. And the file was decoded with `KnownFields`, although a project file is allowed to be partial, so a valid project-only key aborted the release rather than being ignored. The global configuration now comes from `-c`, then the operator's home directory, then AutoBump's published default; a per-project `.autobump.yaml` is merged on top of it through the path that sanitizes it, and is never read as the global configuration. An operator who kept theirs in the working directory rather than in `$HOME` should name it with `-c`.

### Removed

- **BREAKING CHANGE:** removed the `local` subcommand. `autobump .` and `autobump /path/to/repo` are the single-repository form, and `autobump` with no arguments still prints help. `autobump local` is kept hidden and deprecated rather than deleted outright: without it the bare word would fall through to the root command's positional argument and be read as a path, so the error would say that `./local` does not exist rather than that the command is gone.
- removed a nil dereference in the default-configuration fallback, reachable when the published default was named with `-c` and carried no `languages` key. The branch that held it is gone: the built-in defaults are the base of every run, so the languages map can no longer be empty by the time it is read.

### Security

- kept the decision to run a package manager with the operator. `refresh` and `cleanup_stale_branches` are read from a repository's own `.autobump.yaml` only when they are `false`: a project may switch either **off**, never on. Owning the argv was only half of what made `refresh_commands` untrusted — the other half is whether anything is executed at all, and a package manager resolving a lockfile still runs what the cloned repository supplies. The recipes now say so too: pnpm calls `.pnpmfile.cjs` hooks during resolution (`--ignore-pnpmfile`) and Yarn's launcher execs whatever `yarnPath` in the project's own `.yarnrc.yml` names (`YARN_IGNORE_PATH=1`), and no `--ignore-scripts` reaches either. The cleanup half is a bug in its own right: `--skip-cleanup` is applied before the project layer, so an enable arriving there silently overrode the flag and deleted branches the operator had asked to keep.
...
- fixed `TestVersionCommand` racing against itself. Both of its cases assign the `AutobumpVersion` package global and swap `os.Stdout`, which is process-wide, while the function declared `t.Parallel()`; the race detector reports it as soon as the file is visible. It is serial at both levels now, and every other test that cannot be parallel says why in a comment on the function rather than in a `//nolint:tparallel` or `//nolint:paralleltest` directive, where no reader looks — `paralleltest` is not even an enabled linter here, so those two directives were inert
...
- changed the Go module dependencies to their latest versions
- refreshed `.github/copilot-instructions.md` to correct the Go version to `1.26.4`
...
- changed the Go module dependencies to their latest versions
- refreshed `.github/copilot-instructions.md` to remove phantom `DeduplicateEntries` and `UpdateSection` functions, add real changelog functions (`ProcessNewChangelog`, `SortChangelogEntries`, `IsChangelogUnreleasedEmpty`), and correct `changelog.go` description
...
- changed `cliforge` import paths to match the new `pkg/` restructured package layout
- changed the Go module dependencies to their latest versions
...
- changed `gitforge` dependency to `v0.6.0`, picking up fixes for GPG passphrase prompt in CI and branch checkout with unstaged changes
...
- removed redundant `branchExists` helper in Azure DevOps adapter, using `gitutil.CheckBranchExists` instead
